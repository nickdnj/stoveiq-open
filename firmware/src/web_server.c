...
"<input type=checkbox id=flipChk onchange=toggleFlip()> Flip Top/Bottom</label>\n"
"  <label style='display:flex;align-items:center;gap:8px;margin-top:4px;cursor:pointer'>"
"<input type=checkbox id=mirrorChk onchange=toggleMirror()> Mirror Left/Right</label>\n"
...
"<script>\n"
/* State */
"let useFahrenheit=localStorage.getItem('siq_unit')==='F',frameCount=0,lastFpsTime=Date.now();\n"
"let flipDisplay=localStorage.getItem('siq_flip')!=='false';\n"
"let mirrorDisplay=localStorage.getItem('siq_mirror')!=='false';\n"
...
"el.value=Math.round(parseFloat(el.value)*9/5+32)})}\n"
"function toggleFlip(){flipDisplay=!flipDisplay;"
"localStorage.setItem('siq_flip',flipDisplay?'true':'false')}\n"
"function toggleMirror(){mirrorDisplay=!mirrorDisplay;"
"localStorage.setItem('siq_mirror',mirrorDisplay?'true':'false')}\n"
"document.getElementById('flipChk').checked=flipDisplay;\n"
"document.getElementById('mirrorChk').checked=mirrorDisplay;\n"
...
"      }\n"
"      if(flipDisplay)temps.reverse();\n"
"      if(mirrorDisplay){for(let r=0;r<24;r++){let row=temps.slice(r*32,(r+1)*32);"
"row.reverse();for(let c=0;c<32;c++)temps[r*32+c]=row[c]}}\n"
"      const range=mx-mn||1;\n"
"      for(let i=0;i<768;i++){\n"
...
"    /* Transform display coords back to raw sensor coords */\n"
"    const b=calBurners.map(b=>{\n"
"      let r=b.r,c=b.c;\n"
"      if(flipDisplay){r=23-r;c=31-c}\n"
"      if(mirrorDisplay){c=31-c}\n"
"      return{r:Math.round(r),c:Math.round(c),rad:b.rad};\n"
"    });\n"
